# Complete 4-Column Summary Table of All 276 FSSPEC & Filesystem Methods
...
| :--- | :---: | :--- | :--- |
| **`open`** | **1460** | `pytorch/pytorch`, `ray-project/ray`, `Lightning-AI/pytorch-lightning` | Return a file-like object from the filesystem (`fs.open(path, mode)`) |
| **`fsspec.open`** | **51** | `intake/intake`, `huggingface/datasets`, `pandas-dev/pandas` | fsspec module: Return a file-like object from the filesystem (`fs.open(path, mode)`) |
| **`fs.open`** | **46** | `dask/dask`, `iterative/dvc`, `Lightning-AI/pytorch-lightning` | Return a file-like object from the filesystem (`fs.open(path, mode)`) |
| **`fs.exists`** | **30** | `Lightning-AI/pytorch-lightning`, `iterative/dvc`, `dask/dask` | Checking existence of a file or directory node on local or remote filesystem |
...
| **`fs.join`** | **26** | `iterative/dvc`, `pytorch/pytorch` | Cross-platform abstract POSIX path joining without OS separator assumptions |
| **`fs.info`** | **24** | `iterative/dvc`, `intake/intake`, `huggingface/datasets` | Give details and metadata dictionary of entry at path (`size`, `type`, `created`, `mtime`) |
...
| **`fs.isfile`** | **16** | `huggingface/datasets`, `iterative/dvc`, `Lightning-AI/pytorch-lightning` | Verify whether a target path resolves to a leaf file node (not a directory) |
...
| **`fs.ls`** | **11** | `iterative/dvc`, `intake/intake`, `pytorch/torchtitan` | List direct children of a directory (`detail=False` for paths, `detail=True` for info dicts) |
| **`fsspec.filesystem`** | **10** | `intake/intake`, `kedro-org/kedro`, `huggingface/datasets` | fsspec module: Instantiating filesystem driver class by protocol string (e.g. `fsspec.filesystem('s3')`) |
| **`fs.makedirs`** | **10** | `Lightning-AI/pytorch-lightning`, `iterative/dvc`, `huggingface/datasets` | Recursively create directory tree hierarchy (`exist_ok=True`) |
...
| **`fsspec.core.url_to_fs`** | **8** | `intake/intake`, `pandas-dev/pandas`, `ray-project/ray` | Decomposing protocol URI string (`s3://...`, `gs://...`) into abstract `(filesystem, path)` tuple |
...
| **`self._fs.ls`** | **8** | `Lightning-AI/pytorch-lightning`, `kedro-org/kedro`, `huggingface/datasets` | List direct children of a directory (`detail=False` for paths, `detail=True` for info dicts) |
...
| **`fs.normpath`** | **7** | `iterative/dvc` | Normalizing redundant dot/double-dot and slash segments in paths |
...
| **`fs.rm`** | **6** | `Lightning-AI/pytorch-lightning`, `dask/dask`, `pytorch/torchtitan` | Delete files or directory trees (`recursive=True/False`) |
...
| **`self._fs.open`** | **6** | `Lightning-AI/pytorch-lightning`, `kedro-org/kedro`, `huggingface/datasets` | Return a file-like object from the filesystem (`fs.open(path, mode)`) |
...
| **`self.fs.ls`** | **5** | `Lightning-AI/pytorch-lightning`, `intake/intake`, `iterative/dvc` | Instance method: List direct children of a directory (`detail=False` for paths, `detail=True` for info dicts) |
...
| **`self._fs.makedirs`** | **5** | `Lightning-AI/pytorch-lightning`, `huggingface/datasets` | Recursively create directory tree hierarchy (`exist_ok=True`) |
| **`self.fs.open`** | **5** | `apache/arrow`, `pytorch/pytorch` | Instance method: Return a file-like object from the filesystem (`fs.open(path, mode)`) |
...
| **`fs.get_file`** | **4** | `iterative/dvc`, `huggingface/datasets` | Download a single remote file to local target filename path |
...
| **`self.fs.isfile`** | **4** | `apache/arrow`, `iterative/dvc` | Instance method: Verify whether a target path resolves to a leaf file node (not a directory) |
...
| **`fsspec.get_fs_token_paths`** | **3** | `intake/intake`, `pydata/xarray`, `huggingface/datasets` | fsspec module: Parsing URL path string into `(fs, fs_token, paths)` for distributed serialization |
| **`self.repo.fs.relpath`** | **3** | `iterative/dvc` | Calculating relative path string from a reference parent or root directory |
| **`self.fs.info`** | **3** | `iterative/dvc`, `Lightning-AI/pytorch-lightning`, `apache/arrow` | Instance method: Give details and metadata dictionary of entry at path (`size`, `type`, `created`, `mtime`) |
...
| **`self.fs.makedirs`** | **3** | `iterative/dvc`, `pytorch/pytorch`, `Lightning-AI/pytorch-lightning` | Instance method: Recursively create directory tree hierarchy (`exist_ok=True`) |
...
| **`fs_kwargs.get`** | **3** | `iterative/dvc` | Bulk batch downloading of remote cloud or distributed files to local directory disk |
...
| **`fs.listdir`** | **2** | `huggingface/datasets`, `Lightning-AI/pytorch-lightning` | Alias of `ls`; list raw names inside target directory node |
| **`self._fs.rmdir`** | **2** | `huggingface/datasets`, `Lightning-AI/pytorch-lightning` | Remove an empty directory container node |
| **`self._fs.rm_file`** | **2** | `huggingface/datasets`, `Lightning-AI/pytorch-lightning` | Delete a single leaf file node from storage driver |
| **`self._fs.rm`** | **2** | `huggingface/datasets`, `Lightning-AI/pytorch-lightning` | Delete files or directory trees (`recursive=True/False`) |
...
| **`self._fs.move`** | **2** | `ray-project/ray`, `apache/arrow` | Alias of `mv`; move file(s) from one location to another |
...
| **`fs_kwargs.copy`** | **1** | `iterative/dvc` | Copy file(s) within two locations on the filesystem without downloading to local disk |
| **`Git.init(self.fs_path).close`** | **1** | `iterative/dvc` | Close file stream handle and release buffer resources |
...
| **`self._fs.mkdir`** | **1** | `huggingface/datasets` | Create single directory container node at path (`create_parents=False`) |
| **`self._fs.info`** | **1** | `huggingface/datasets` | Give details and metadata dictionary of entry at path (`size`, `type`, `created`, `mtime`) |
...
| **`Path(fs_path, _TUNER_PKL).as_posix`** | **1** | `ray-project/ray` | Format path with standardized POSIX forward slash separators |
...
| **`self.fs.find`** | **1** | `apache/arrow` | Instance method: Recursively find all file paths inside a directory subtree matching optional criteria |
...
| **`fs.mkdir`** | **1** | `apache/arrow` | Create single directory container node at path (`create_parents=False`) |
